#!/usr/bin/env python3
"""Cumulative-only merge-aware audit: does treating a taxid as 'existing at snapshot T' only if
it (or a predecessor id that later merged into it) appears in nodes.dmp(T) change the
per-version library-size trajectory and the 13-way fixed-library intersection? This produced
the article's original quantification on the 15,359-identifier tagged library (218-269
recovered identifiers per snapshot; 13-way intersection growing from 5,270 to 5,518 once
merge-corrected) -- kept here for the comparability check reported in the Analysis Protocol
and in snapshot_aware_validation.py, not as the current source of Table 2. Table 2 itself is
now generated by snapshot_aware_validation.py, which checks each snapshot's own historical
record directly instead of only the latest one (recommendation 2), recovering reversal and
single-snapshot cases this cumulative-only method misses.
